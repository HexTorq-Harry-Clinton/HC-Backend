// routes/WishlistItems.js (HARRY_CLINTON) - BASIC CRUD (5 endpoints)
...
========================================= */
const FIELD_TYPES = {
  wishlist_item_id: { type: sql.VarChar, maxLength: 36 },
  wishlist_id: { type: sql.VarChar, maxLength: 36 },
  product_id: { type: sql.VarChar, maxLength: 36 },
  product_variant_id: { type: sql.VarChar, maxLength: 36 },

  added_at: { type: sql.DateTime },
...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (!id) {
      return res
        .status(400)
        .json({ success: false, message: 'wishlist_item_id required' });
    }
    ...
    const request = pool
      .request()
      .input('wishlist_item_id', FIELD_TYPES.wishlist_item_id.type, id);
    ...
    console.error('Wishlist Items GET BY ID error:', err);
    res.status(500).json({ success: false, message: err.message });
  }
});
...
router.post('/', async (req, res) => {
  try {
    ...
    if (!data.wishlist_id || !data.product_id) {
      return res.status(400).json({
        success: false,
        message: 'wishlist_id, product_id required'
      });
    }
    ...
    const result = await request.query(`
      INSERT INTO dbo.tbl_wishlist_items (${cols.join(',')})
      OUTPUT INSERTED.*
      VALUES (${vals.join(',')});
    `);
    ...
    console.error('Wishlist Items POST error:', err);
    res.status(500).json({ success: false, message: err.message });
  }
});
...
router.put('/', async (req, res) => {
  try {
    ...
    if (!data.wishlist_item_id) {
      return res.status(400).json({
        success: false,
        message: 'wishlist_item_id required'
      });
    }
    ...
    request.input(
      'wishlist_item_id',
      FIELD_TYPES.wishlist_item_id.type,
    ...
    console.error('Wishlist Items PUT error:', err);
    res.status(500).json({ success: false, message: err.message });
  }
});
...
router.delete('/', async (req, res) => {
  try {
    const { wishlist_item_id, luu } = req.body;
    ...
    const request = pool
      .request()
      .input('wishlist_item_id', FIELD_TYPES.wishlist_item_id.type, wishlist_item_id);
    ...
    res.json({
      success: true,
      message: 'Wishlist item deleted (soft)'
    });
  } catch (err) {
    console.error('Wishlist Items DELETE error:', err);
    res.status(500).json({ success: false, message: err.message });
  }
});
