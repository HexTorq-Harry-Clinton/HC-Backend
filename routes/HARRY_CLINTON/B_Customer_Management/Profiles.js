...
// routes/Profiles.js (HARRY_CLINTON) - BASIC CRUD (5 endpoints)
...
========================================= */
const FIELD_TYPES = {
  profile_id: { type: sql.VarChar, maxLength: 36 },
  user_id: { type: sql.VarChar, maxLength: 36 },

  fullname: { type: sql.VarChar, maxLength: 255 },
  emailid: { type: sql.VarChar, maxLength: 255 },
  mobile_number: { type: sql.VarChar, maxLength: 50 },
  gender: { type: sql.VarChar, maxLength: 20 },
  dateofbirth: { type: sql.Date },
  profile_url: { type: sql.VarChar, maxLength: 1000 },

  lastloginat: { type: sql.DateTime },
...
router.get('/:id', async (req, res) => {
  try {
    ...
    if (!id) {
      return res
        .status(400)
        .json({ success: false, message: 'profile_id required' });
    }
    ...
    const request = pool
      .request()
      .input('profile_id', FIELD_TYPES.profile_id.type, id);
    ...
    console.error('Profiles GET BY ID error:', err);
    res.status(500).json({ success: false, message: err.message });
  }
});
...
router.post('/', async (req, res) => {
  try {
    ...
    if (!data.user_id || !data.fullname || !data.emailid) {
      return res.status(400).json({
        success: false,
        message: 'user_id, fullname, emailid required'
      });
    }
    ...
    const result = await request.query(`
      INSERT INTO dbo.tbl_profiles (${cols.join(',')})
      OUTPUT INSERTED.*
      VALUES (${vals.join(',')});
    `);
...
router.delete('/', async (req, res) => {
  try {
    ...
    const request = pool
      .request()
      .input('profile_id', FIELD_TYPES.profile_id.type, profile_id);
